...
void	r3d_raycast_world(t_r3d *r3d, t_map *map, t_vars *vars)
{
	...
	while (x < r3d->width)
	{
		...
		while (hit == 0)
		{
			...
			if (mapX < 0 || mapX >= map->width || mapY < 0 || mapY >= map->height)
				break ;
			if ((map->tiles[mapX + mapY * map->width] >= TILE_FULL && map->tiles[mapX + mapY * map->width] <= TILE_9)
				|| map->tiles[mapX + mapY * map->width] == TILE_DOOR) hit = 1;
		}
		...
		if (hit)
		{
			...
			if (map->tiles[mapX + mapY * map->width] == TILE_DOOR)
				...
				texture = texture_for_wall(map, side,
					v2i(vars->map.player->base.transform.position.x, vars->map.player->base.transform.position.z), v2i(mapX, mapY));
			double	wall_x;
	...
	for(size_t i = 0; i < ft_vector_size(vars->map.entities); i++)
	{
		...
		t_image	*image = fake_player_get_image(fp, vars);
