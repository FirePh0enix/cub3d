...
/*   Updated: 2024/03/03 13:47:31 by ledelbec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	const t_mat4	projection = mat4_projection(r3d->fov, 1280, 720);
	const t_mat4	rotation = mat4_z_rot(r3d->rot_z);
	const t_mat4	rotation2 = mat4_x_rot(r3d->rot_z);
	size_t			i;
	t_tri			tri;
	...
	while (i < mesh->vertex_count)
	{
		...
		tri = (t_tri){
			mat4_multiply_v3(rotation2, tri.v0),
			mat4_multiply_v3(rotation2, tri.v1),
			mat4_multiply_v3(rotation2, tri.v2),
		};
		...
		stri.v0.x = (int)tri.v0.x; stri.v0.y = (int)tri.v0.y;
		stri.v1.x = (int)tri.v1.x; stri.v1.y = (int)tri.v1.y;
		stri.v2.x = (int)tri.v2.x; stri.v2.y = (int)tri.v2.y;

		if (opts->draw_wireframe)
			draw_triangle_wireframe(r3d, tri, opts->wireframe_color);
		else
			r3d_fill_triangle(r3d, stri, color);
		i += 3;
	}
}
